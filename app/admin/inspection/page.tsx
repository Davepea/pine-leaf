...
import InspectionTable from '@/components/admin/table/InspectionTable'
        ...
        <div>
            <div className="md:px-10 px-6 w-full h-screen overflow-y-scroll overflow-x-hidden flex flex-col gap-5">
                ...
                <div className="flex justify-between items-center">
                    <form onSubmit={(e) => e.preventDefault()} className='h-[40px] md:w-[386px] w-[185px]'>
                        <label htmlFor="search" className='md:px-5 px-3 border border-[#2F5318]/20 h-full w-full rounded-[10px] flex items-center text-[#000000]/40'>
                            <input type="text" name="search" id="search" placeholder='Search inspection' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className='h-[40px] w-full outline-none border-none placeholder:text-[#000000]/80' />
                            <MdSearch size={20} />
                        </label>
                    </form>
                ...
                <div className="py-5 gap-5">
                    <InspectionTable searchTerm={searchTerm} />
                </div>
            </div>
